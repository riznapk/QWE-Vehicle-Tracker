    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                # You can perform additional checks here, such as checking if the user has the necessary permissions to access the resource.
...
def userData():
    ...
    if request.method == 'POST':
        ...
        # if not re.match(r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[^ ]{6,}$', str(userPassword)):
        #     print("passwor")
        #     return jsonify({
        #         'status': 'Password must contain at least 6 characters, one uppercase letter, one lowercase letter, one number and one special character.',
        #         'message': 'failure'
        #     })
